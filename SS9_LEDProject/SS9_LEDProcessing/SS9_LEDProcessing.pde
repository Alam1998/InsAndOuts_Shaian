// Shaian Khan, title: SS9_LEDPRocessing.

// Drag the mouse pointer over the moon to activate the LED effects on the auduino board. 
// Drag the mouse pointer near the exit button (the right top corner) to activate a different light effect. 
...
image(background, 0, 0);
  ...
  println (val);
    enemy();
      ...
      else if( myPort.available() > 200 || myPort.available() < 255) {
image(background, 0, 0);
  enemy();
   val2 = myPort.read();
  stroke(255);
  fill(#0BD2E3 ); 
  ellipse(portalW/5, portalH/8, val2, val2);
...
//mouse location controls communication to Serial 
  if (mouseY > width/5 && mouseY < height) {  
